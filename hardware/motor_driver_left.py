...
I2C_ADDR  = Addresses.LEFT_DRIVER
...
async def poll_loop():
    ...
    while True:
        try:
            ...
            left_driver_data["motor_current"] = await read_motor_current()
            left_driver_data["velocity"] = await read_velocity()
            left_driver_data["status"]   = await read_status()
        except Exception as e:
            print(f"Left driver error: {e}")
